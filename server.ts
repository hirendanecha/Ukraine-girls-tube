...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/ukrainian-girls/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'dist/ukrainian-girls/browser'),
        'index.html'
      )
    )
    .toString();
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            ...
            description: 'ukrainiangirls.tube',
          ...
          const talent = {
            name: post?.title || post?.albumname || 'ukrainian-girls Post',
          ...
          const talent = {
            name: `ukrainiangirls.tube Research ${group?.PageTitle}`,
